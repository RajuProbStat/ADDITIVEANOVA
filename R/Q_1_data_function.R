library(Iso)
library(matlib)
#' Evaluate the critical point and observed value of the test statistic Q_1 from a given data
#'
#' @param data a real data set for a Two-way additive ANOVA model
...
#'
#' @examples
#' data<-read.csv("https://raw.githubusercontent.com/RajuProbStat/AdditiveANOVA/main/Alcohol_real_data.csv")
#' Q_1_data(data,3,2,0.05)
Q_1_data<-function(data,a,b,size){
  n<-array(NA,dim=c(a,b))
  N<-unlist(rbind(lapply(data,length)))
  new<-unlist(rbind(sapply(data,function(x) sum(is.na(x)))))
  for(i in 1:a)
  {
    for(j in 1:b)
    {
      n[i,j]<-N[(i-1)*b+j]-new[(i-1)*b+j]
    }
  }
  ...
  var1<-function(vec)
  {
    ...
    vec[is.na(vec)]=mean
    var<-((n-1)/(n-m-1))*var(vec)
    return(var)
  }
  var_data<-unlist(rbind(lapply(data,var1)))
  mean<-array(NA,dim=c(a,b))
  var<-array(NA,dim=c(a,b))
  for(i in 1:a)
  {
    for(j in 1:b)
    {
      mean[i,j]<-mean_data[(i-1)*b+j]
      var[i,j]<-var_data[(i-1)*b+j]
    }
  }
  func_MLE<-function(a,b,n,mean,var){
    ...
    for(j in 1:b)
    {
      mean_beta_0A[j]<-mean(mean[,j])            #initialization of beta
    }
    beta_0_0A<-mean_beta_0A
